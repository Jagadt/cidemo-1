...
import subprocess
USERS_LIST = {
'scalrdemo1@mail.ru': 10,
'scalrdemo2@mail.ru': 11,
'scalrdemo3@mail.ru': 12
}
DEV_FARM_ID = 9800014
PRIVATE_KEY = '/var/lib/jenkins/.ssh/devel.pem'
STAGING_IP = "104.197.18.43"
STAGING_FARM_ID = "9800019"
...
with open('/var/lib/jenkins/scripts/farm_update.json', 'r') as f:
proto = json.load(f)
...
subprocess.check_call('scalr-ctl farms update --farmId %s --stdin < /tmp/farm_update_%s.json' %
(farm_id, farm_id), shell=True)
...
out = json.loads(subprocess.check_output(['scalr-ctl', 'farms', 'list-servers', '--farmId', str(farm_id),
'--json']))
...
subprocess.check_call("scp -i %s /tmp/%s.tar.gz scalr@%s:/tmp/" % (key, hash, server_ip),
shell=True)
subprocess.check_call("ssh -i %s scalr@%s \"sudo rm -rf /var/www/html/* && sudo tar xzf
/tmp/%s.tar.gz -C /var/www/html\"" % (key, server_ip, hash), shell=True)
def run_tests(server_ip):
print('Run tests ')
subprocess.check_output("ssh -i %s scalr@%s \"sudo python /var/www/html/tests.py\"" %
(PRIVATE_KEY, server_ip), shell=True)
...
farm_name = os.environ.get('GIT_BRANCH').replace('/', '-') + '-%s' % hash[:7]
...
out = json.loads(subprocess.check_output("scalr-ctl farms get --farmId %s --envId %s --json" %
(farm_id, env), shell=True))
with open('/tmp/rename_farm_%s.json' % farm_name, 'w+') as f:
out['data']['name'] = farm_name
f.write(json.dumps(out['data']))
subprocess.check_call("scalr-ctl farms update --envId %s --farmId %s --stdin <
/tmp/rename_farm_%s.json" % (env, farm_id, farm_name), shell=True)
